...
#pragma region public_methods
				...
				pplx::task<std::shared_ptr<Stormancer::IP2PScenePeer>> initializeP2P(std::string p2pToken, bool openTunnel, pplx::cancellation_token ct)
				{
					...
					if (!scene)
					{
						...
						STORM_RETURN_TASK_FROM_EXCEPTION(std::runtime_error("scene deleted"), std::shared_ptr<Stormancer::IP2PScenePeer>);
					}
					...
					else // Client
					{
							...
							.then([wThat, ct, openTunnel](std::shared_ptr<IP2PScenePeer> p2pPeer)
						{
							...
							{
								STORM_RETURN_TASK_FROM_EXCEPTION(std::runtime_error("Service destroyed"), std::shared_ptr<IP2PScenePeer>);
							}
						})
				...
				pplx::task<void> reset(pplx::cancellation_token ct = pplx::cancellation_token::none())
				{
					...
					if (!scene)
					{
						STORM_RETURN_TASK_FROM_EXCEPTION(std::runtime_error("Scene deleted"), void);
					}
				...
				pplx::task<Packetisp_ptr> sendGameResults(const StreamWriter& streamWriter, pplx::cancellation_token ct)
				{
					...
					if (!scene)
					{
						STORM_RETURN_TASK_FROM_EXCEPTION(std::runtime_error("Scene deleted"), Packetisp_ptr);
					}
