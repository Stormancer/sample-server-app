...
namespace Stormancer
{
	...
	namespace GameFinder
	{
		...
		public:
			...
			virtual pplx::task<std::unordered_map<std::string, int>> getMetrics(const std::string& gameFinderName) = 0;
		...
		namespace details
		{
			...
			public:
				...
				pplx::task<std::unordered_map<std::string, int>> getMetrics()
				{
					return _rpcService->rpc<std::unordered_map<std::string, int>>("gamefinder.getmetrics");
				}
			...
			public:
				...
				pplx::task<std::unordered_map<std::string, int>> getMetrics(const std::string& gameFinderName) override
				{
					return getGameFinderContainer(gameFinderName)
						.then([](std::shared_ptr<GameFinderContainer> gameFinderContainer)
					{
						return gameFinderContainer->service()->getMetrics();
					});
				}
			...
			private:
				...
				pplx::task<std::shared_ptr<GameFinderContainer>> connectToGameFinderImpl(std::string gameFinderName)
				{
					...
					if (!users)
					{
						STORM_RETURN_TASK_FROM_EXCEPTION(std::runtime_error("Users service destroyed."), std::shared_ptr<GameFinderContainer>);
					}
